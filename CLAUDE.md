...
└── services/
    ├── upstage.py       # 번역 + Mock 에이전트: Upstage solar-pro2 단일 호출
    └── solar_agent.py   # 프로덕션 에이전트: Upstage solar-pro2 + Tavily 웹 검색
```

번역(`translate.py`)은 `USE_MOCK` 값과 무관하게 항상 `upstage.translate()`를 사용한다.
에이전트(`agent.py`)는 `USE_MOCK=true`면 `upstage.stream_agent()`로, `false`면 `solar_agent.stream_agent_response()`로 라우팅된다.

### 프로덕션 에이전트 파이프라인 (`solar_agent.py`)

`USE_MOCK=false` 시 동작하는 4단계 파이프라인:

1. **주장 파악 (decision)**: 최신 교환에서 검증할 사실적 주장을 짧은 구절로 요약
2. **쿼리 계획 (`_plan_queries`)**: 주장 검증에 필요한 단서별로 타겟 검색 쿼리 1-2개 생성
3. **Tavily 검색**: 쿼리별 병렬 검색, 결과 합산
4. **추론·검증 (verify)**: 검색 결과를 단서로 추론 → 결론 도출. 검색 실패/불충분 시 자체 지식 fallback
